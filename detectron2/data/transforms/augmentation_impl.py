...
class NoiseTransform(Transform):
    ...
    def apply_image(self, img: np.ndarray) -> np.ndarray:
        noise = np.random.normal(scale=self.std, size=(img.shape[0], img.shape[1], 1))
...
class RandomNoise(Augmentation):
    ...
    def get_transform(self, image):
        # randomly select std:
        std = np.random.uniform(0, self.std)
        return NoiseTransform(std)
...
class RandomBlur(Augmentation):
    ...
    The kernel size for the Gaussian blur is uniformly sampled from
    (kernel_size_min, kernel_size_max). Larger kernel sizes result in more blurring.

    See: https://pytorch.org/vision/stable/transforms.html#torchvision.transforms.functional.gaussian_blur
    """

    def __init__(self, kernel_size_min, kernel_size_max):
        """
        Args:
            kernel_size_min (int): Minimum kernel size for the Gaussian blur.
                                   Must be an odd integer ≥ 1.
            kernel_size_max (int): Maximum kernel size for the Gaussian blur.
                                   Must be an odd integer ≥ kernel_size_min.
        """
        super().__init__()
        self._init(locals())
        assert kernel_size_min >= 1 and kernel_size_min % 2 == 1, \
            "kernel_size_min must be an odd integer ≥ 1."
        assert kernel_size_max >= kernel_size_min and kernel_size_max % 2 == 1, \
            "kernel_size_max must be an odd integer ≥ kernel_size_min."

    def get_transform(self, image):
        # Randomly sample an odd kernel size between kernel_size_min and kernel_size_max
        kernel_size = np.random.randint(
            self.kernel_size_min, self.kernel_size_max + 1
        )
...
class BlurTransform(Transform):
    """
    Applies Gaussian blur with a specified kernel size.
    """
    ...
    def apply_image(self, image):
        ...
        if not isinstance(image, np.ndarray) or image.ndim != 3:
            raise ValueError("Input must be a numpy array of shape (H, W, C).")
        blurred_image = cv2.GaussianBlur(image, (self.kernel_size, self.kernel_size), 0)
        return blurred_image

    def apply_coords(self, coords):
        """
        This is a no-op for blurring, as it does not affect coordinates.
        """
        return coords
